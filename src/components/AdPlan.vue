      ...
      <table>
        ...
        <tbody v-for="userYearCareerInfo in userCareer.careerPlanForYears">
          <tr v-for="userCareerInfo in userYearCareerInfo.careerPlans">
            ...
            <td width="10%" class="adp-opr">
              <!-- <router-link v-bind:to="'/student/' + userCareerInfo.id" class="font-war">查看用户</router-link> -->
              <a href="javascript:void(0)" v-on:click="passUserCareer(userCareerInfo)" class="font-suc" v-if="userCareerInfo.state == 0">通过</a>
              <a href="javascript:void(0)" v-on:click="backUserCareer(userCareerInfo)" class="font-dan" v-if="userCareerInfo.state == 0">退回</a>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    passUserCareer: function (userCareerInfo) {
      this.verify.careerPlanId = userCareerInfo.id
      this.verify.state = 1
    ...
    backUserCareer: function (userCareerInfo) {
      if (userCareerInfo != null) {
        this.verify.careerPlanId = userCareerInfo.id
        this.userCareerAdd.userIds = [userCareerInfo.userId]
      }
    ...
    verifyUserCareer: function (data) {
      var self = this
      axios.post(CONFIG.baseURL + 'careerPlan/verify', CONFIG.postHttpDataWithToken(data)).then(function (res) {
    ...
    getUserCareer: function (data) {
      var self = this
      axios.get(CONFIG.baseURL + 'careerPlan/getCareePlanList?token=' + CONFIG.getToken() + CONFIG.getHttpData(data)).then(function (res) {
        if (res.data.callStatus === 'SUCCEED') {
          self.userCareer = []
